function [vars] = update_RY_regR_L1_pro_lin(vars, params)
...
R1 = soft_shrinkage(vars.R - (RR.data - Delta.data) / ( norm(Uc)^2 * norm(Ur)^2 ), ...
    alpha * norm(vars.A, 'fro') * norm(vars.B, 'fro') / mu);
